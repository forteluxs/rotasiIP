...
func Run(opt *common.Options) {
	...
	// Init rate limiter — RateLimit takes precedence over Delay
	switch {
	case opt.RateLimit > 0:
		limiter = ratelimiter.NewFromRPS(opt.RateLimit)
	case opt.Delay > 0:
		limiter = ratelimiter.NewFromDelay(opt.Delay)
	default:
		limiter = ratelimiter.NewFromRPS(0) // disabled
	}

	if opt.UIAddress != "" {
		go func() {
			log.Infof("Dashboard running on http://%s", opt.UIAddress)
			if err := dashboard.Run(opt.UIAddress, opt); err != nil {
				log.Errorf("Dashboard error: %s", err)
			}
		}()
	}
